...
public class CenterServer extends InterfaceCenterPOA
{
	...
	public String createTRecord(String mi, String fn, String ln, String ad, String ph, String sp,String lo)
	{
		...
		String res="";
		if(UdpTaskThread.getThisTask().getSucno()==0)
		{
			res="successfully";
			Log.writeLog(time, mi.trim().substring(0, 3), mi, "Create_New_Teacher_Record", res, newtr.getNo().trim());
		}
		else
		{
			res="failure";
			Log.writeLog(time, mi.trim().substring(0, 3), mi, "Create_New_Teacher_Record", res, "");
		}
	...
	public String createSRecord(String mi, String fn, String ln, boolean s, String[] CourseRegistered)
	{
		...
		{
			res="successfully";
			Log.writeLog(time, mi.trim().substring(0, 3), mi, "Create_New_Student_Record", res, newsr.getNo().trim());
		}
		else
		{
			res="failure";
			Log.writeLog(time, mi.trim().substring(0, 3), mi, "Create_New_Student_Record", res, "");
		}
	...
	public String getRecordCounts(String mi)
	{
		...
		{
			res="successfully";
			Log.writeLog(time, mi.trim().substring(0, 3), mi, "Query_Records_Count", res, resulttt);
		}
		else
		{
			res="failure";
			Log.writeLog(time, mi.trim().substring(0, 3), mi, "Query_Records_Count", res, "");
		}
	...
	public String editRecord(String mi, String recordID, String fieldname, String[] newvalue)
	{
		...
		else// record in local
		{
			if (recordID.charAt(0) == 'T')// teacher
			{
				...
				{
					res="successfully";
					Log.writeLog(time, mi.trim().substring(0, 3), mi, "Edit_Record", "Edit_"+res, "The_new_value_of_"+fieldname.trim()+"_is_"+al.get(0).trim());
				}
				else
				{
					res="failure";
					Log.writeLog(time, mi.trim().substring(0, 3), mi, "Edit_Record", "Edit_"+res, "");
				}
			...
			else// student
			{
				...
				{
					res="successfully";
					Log.writeLog(time,mi.trim().substring(0, 3), mi, "Edit_Record", "Edit_"+res, "The_new_value_of_"+fieldname.trim()+"_is"+remark.trim());
				}
				else
				{
					res="failure";
	...
	public void StransferRecord(String site, byte[] record, String rid, String mi, String fromN)
	{
		...
		else
		{
			...
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd--HH:mm:ss");
			String time = sdf.format(d);
			//System.out.println(site.trim().substring(0, 3));
	...
	public String MtransferRecord(String mid, String rid, String remoteName)
	{
		...
		{
			res="successfully";
			Log.writeLog(time, mid.trim().substring(0, 3), mid.trim(), "Transfer_Record_:" + rid.trim()+"_To_"+remoteName.trim(), res, "");
		}
		else
		{
			res="failure";
